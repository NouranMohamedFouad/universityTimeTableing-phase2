...
public class StudentScheduleTable extends Composite {
		...
		iHeader.addButton("registration", MESSAGES.buttonRegistration(), new ClickHandler() {
			...
			public void onClick(ClickEvent e) {
				EnrollmentTable.showCourseRequests(iStudent, iOnline, new AsyncCallback<Boolean>() {
					...
					{
						//adding this exception because it was empty
	...
	public void populate(Collection<CourseAssignment> data) {
		...
		iHeader.clearMessage();
		//remove<WebTable.Row> to be <>
		ArrayList<WebTable.Row> rows = new ArrayList<>();
		for (ClassAssignmentInterface.CourseAssignment course: data) {
			if (course.isAssigned()) {
				...
				for (ClassAssignmentInterface.ClassAssignment clazz: course.getClassAssignments()) {
					...
					WebTable.Cell subjectCell;
					if (firstClazz) {
					    if (course.isFreeTime()) {
					        subjectCell = new WebTable.Cell(MESSAGES.freeTimeSubject());
					    } else {
					        subjectCell = new WebTable.Cell(course.getSubject());
					    }
					} else {
					    subjectCell = new WebTable.Cell("");
					}

					WebTable.Cell courseCell;
					if (firstClazz) {
					    if (course.isFreeTime()) {
					        courseCell = new WebTable.Cell(MESSAGES.freeTimeCourse());
					    } else {
					        courseCell = new WebTable.Cell(course.getCourseNbr());
					    }
					} else {
					    courseCell = new WebTable.Cell("");
					}
